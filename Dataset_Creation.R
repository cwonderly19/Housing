...
Synth_Practice <- merge(Synth_Practice, levels_approx, by = c("Year", "GEOID"), all = FALSE)
...
practice <- practice[-c(27)]

practice_dcast <- dcast(practice, practice$GEOID ~ practice$Year, value.var = "Placement", fun.aggregate = sum)
practice_dcast[is.na(practice_dcast)] <- 0
...
Synth_Practice <- merge(Synth_Practice, practice, by = c("GEOID", "Year"), all = FALSE)
...
Synth_Data$Control <- ifelse(Synth_Data$Treatment == 0 & Synth_Data$Treated == 0, 1,0)
...
Synth_Data <- merge(Synth_Data, pre_treat, by = c("GEOID", "Year"), all = FALSE)
